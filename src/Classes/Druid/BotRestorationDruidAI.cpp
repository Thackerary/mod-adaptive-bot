...
private:
    ...
    bool MaintainTankHoTs()
    {
        ...
        Aura* bloom = lifebloom ? tank->GetAura(lifebloom) : nullptr;

        // ---- 回春术优先：主坦生命跳板绝不能缺 ----
        // 三花分支每轮巡检都会 return true，若把它排在前面，主坦缺失的回春术将被
        // 三花续订无限抢帧，形成「三花常满、回春永缺」的永久饿死。
        // 因此回春前置，仅在「三花极度濒危」(剩余时间 < 2000ms，断档即掉层) 时
        // 让位给三花续订，其余时刻一律优先补齐回春跳板。
        // 剩余时间 <= 0 的永久光环不参与濒危判定：它永不到期，无需为其让位。
        if (rejuvenation && !tank->HasAura(rejuvenation))
        {
            bool const lifebloomCritical = bloom
                                        && bloom->GetDuration() > 0
                                        && bloom->GetDuration() < 2000;

            if (!lifebloomCritical && TryRejuvenation(tank))
                return true;
        }
    ...
    // =========================================================================
    // P1-c: 野性印记 (脱战长效团队增益维护)
    // -------------------------------------------------------------------------
    // 爪子是 30 分钟长效增益，收益恒定但绝不救命：
    // 严格限定为「仅脱战维护」，战时一律禁闭。
    // (原「战时血线 >= 80% 才补」的软门禁仍会空耗战斗 GCD 去补 30 分钟 buff，
    //  一旦补印途中血线骤降，这一次 GCD 就是实打实的断奶窗口。)
    // =========================================================================
    bool MaintainMarkOfTheWild()
    {
        // 战时禁闭 (硬性)：战斗内任何一帧的 GCD 都必须留给治疗通道
        if (me->IsInCombat())
            return false;
        ...
        // 先择目标：自身优先 (本体缺印会连带削弱承伤与续航)，其次任一缺印队友。
        // 宠物由主人自行管理增益：宠物不参与打地鼠雷达的优先级排序，
        // 为其补印会白白消耗救命 GCD。
        Unit* markTarget = nullptr;
        ...
        // 全员齐全：直接放行，绝不做无意义的拔形态/变形摆动 (每次变形都消耗一次 GCD)
        if (!markTarget)
            return false;

        // 平衡系法术 (野性印记) 在树形态下被引擎硬性拒绝，
        // 若不移除形态，补印会恒定失败并形成每轮巡检的空转。
        // RemoveAurasDueToSpell 非施法动作、不占用 GCD；全员补齐后
        // 由 MaintainTreeOfLife 在下一轮巡检自动变回树形态。
        if (me->GetShapeshiftForm() == FORM_TREE)
            me->RemoveAurasDueToSpell(RestorationDruidSpells::TREE_OF_LIFE);

        if (!CanCast(markTarget, markOfTheWild, true))
            return false;

        return ExecuteSpell(markTarget, markOfTheWild, true);
    }
    ...
    bool TryCleanse()
    {
        ...
        for (Unit* ally : groupSnapshot.allies)
        {
            ...
            // 平衡系驱散在树形态下被引擎硬性拒绝：驱除诅咒 (REMOVE_CURSE) 属平衡系，
            // 若不移除形态，CanCast 会恒定失败，整轮驱散巡检沦为死循环空转。
            // 驱毒术不受形态限制，故仅对诅咒通道拔形态，避免无谓的变形摆动。
            // RemoveAurasDueToSpell 非施法动作、不占用 GCD，驱散可当帧落地。
            if (chosenDispel == removeCurse && me->GetShapeshiftForm() == FORM_TREE)
                me->RemoveAurasDueToSpell(RestorationDruidSpells::TREE_OF_LIFE);
